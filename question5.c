...
int main(){
 ...
 // END FOR 
  ...
  if (A_count==A_max && B_count==B_max && C_count==C_max)
  {
   printf("\nparking capacity of entire campus i full");
  }
  if (A_count>B_count && A_count>C_count)
  {
    printf("\nzone A has highest occupancy");
  }
  else if (B_count>A_count && B_count>C_count)
  {
    printf("\nzone B has highest occupancy");
  }
  else
  {
    printf("\nzone C has highest occupancy");
